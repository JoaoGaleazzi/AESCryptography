...
namespace AESCryptography
{
    internal class AESCrytography
    {
        ...
        public byte[] Decrypt(byte[] encryptedText, byte[] key)
        {
            ...
            List<byte> bytesDecifrados = new List<byte>();

            foreach (byte[] bloco in blocos)
            {
                byte[] blocoDecifrado = DecypherBlock(bloco, keyScheduleWords);
                bytesDecifrados.AddRange(blocoDecifrado);
            }

            return RemoveFilling(bytesDecifrados.ToArray());
        }


        private byte[] DecypherBlock(byte[] bloco, List<byte[]> keyScheduleWords)
        {
            byte[,] state = ByteArrayToStateMatrix(bloco);
        ...
        private byte[,] InvShiftRows(byte[,] state)
        {
            ...
            for (int linha = 0; linha < 4; linha++)
            {
                for (int coluna = 0; coluna < 4; coluna++)
                {
                    result[linha, coluna] = state[linha, (coluna - linha + 4) % 4];
                }
            }
        ...
        private byte[,] InvSubBytes(byte[,] state)
        {
            ...
            for (int linha = 0; linha < 4; linha++)
            {
                for (int coluna = 0; coluna < 4; coluna++)
                {
                    result[linha, coluna] = InvSBox[state[linha, coluna]];
                }
            }
            return result;
        }
        ...
        private static byte[,] InvMixColumns(byte[,] state)
        {
            for (int c = 0; c < 4; c++)
            {
                ...
                state[0, c] = (byte)(GaloisMultiply(0x0E, s0) ^
                                     GaloisMultiply(0x0B, s1) ^
                                     GaloisMultiply(0x0D, s2) ^
                                     GaloisMultiply(0x09, s3));

                state[1, c] = (byte)(GaloisMultiply(0x09, s0) ^
                                     GaloisMultiply(0x0E, s1) ^
                                     GaloisMultiply(0x0B, s2) ^
                                     GaloisMultiply(0x0D, s3));

                state[2, c] = (byte)(GaloisMultiply(0x0D, s0) ^
                                     GaloisMultiply(0x09, s1) ^
                                     GaloisMultiply(0x0E, s2) ^
                                     GaloisMultiply(0x0B, s3));

                state[3, c] = (byte)(GaloisMultiply(0x0B, s0) ^
                                     GaloisMultiply(0x0D, s1) ^
                                     GaloisMultiply(0x09, s2) ^
                                     GaloisMultiply(0x0E, s3));
            }

            return state;
        }

        private byte[] RemoveFilling(byte[] data)
        {
            if (data.Length == 0)
                return data;
            ...
            if (paddingValue < 1 || paddingValue > BLOCK_SIZE)
                return data;

            int newLength = data.Length - paddingValue;
            return data.Take(newLength).ToArray();
        }
        #endregion
        ...
        private List<byte[]> ExpandKey(byte[] key)
        {
            ...
            for (int i = 4; i < 44; i++)
            {
                ...
                if (i % 4 == 0)
                {
                    ...
                    byte[] roundConstant = GerarRoundConstant((i / 4) - 1);
